...
pub trait EnvTrait: Send + 'static {
...
impl<T: EnvTrait> AscHeap for FunctionEnvMut<'_, T> {
    fn raw_new(&mut self, bytes: &[u8]) -> Result<u32, AscError> {
        ...
        // of the node.
        if let Some(memory_allocate) = env.memory_allocate() {
        ...
        let memory = env.get_memory().unwrap();
    ...
    fn read_u32(&self, offset: u32) -> Result<u32, AscError> {
        let mut bytes = [0; 4];
        let env = self.data();
        // assert!(env.get_memory().is_some(), "No memory???");
        let memory = &env.get_memory().unwrap();
